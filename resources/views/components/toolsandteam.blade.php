<section>
    <section class="flex flex-wrap justify-center pt-[5rem] pb-[7rem] px-[2rem]">
        <div class="w-full flex flex-wrap justify-center items-center max-w-[90vw] p-6 bg-[length:100%_100%] rounded-[15px] sm:rounded-[30px] h-[150px] sm:h-[300px]"
            style="background: url('{{ asset('images/background.png') }}'); background-repeat: no-repeat; backgroind-size: cover; background-position: center">
            <h2 class="text-white text-[13px] sm:text-[28px] mb-[1rem] md:text-[36px] font-bold mb-6 text-center">Tools
                And Platforms We Are Good At</h2>
        </div>
        <div class="h-[auto] max-w-[65vw] min-w-[65vw] sm:w-[70vw] mt-[-3rem] sm:mt-[-7rem]">
            <div class="grid grid-cols-1 bg-red h-auto gap-y-4">
                <div
                    class="h-fit bg-[#EFF4FF] flex rounded-[14px] px-[1rem] lg:px-[3rem] xl:px-[10rem] pt-[2rem] pb-[2.5rem] grid gird-cols-1">
                    <div class="flex flex-wrap justify-center mb-[1rem] sm:mb-[2rem]">
                        <h1 class="text-[13px] sm:text-[20px] text-[#012F6A] font-bold"
                            style="font-family: Red Hat Display">Platforms</h1>
                    </div>
                    <div
                        class="flex flex-wrap justify-self-center w-full justify-between grid grid-cols-5 gap-y-4 sm:grid sm:grid-cols-10 sm:gap-y-0">
                        <img class="w-[20px] h-[20px] sm:w-[30px] sm:h-[30px] lg:w-[40px] lg:h-[40px] justify-self-center xl:justify-self-auto"
                            src="images/social/meta.png" alt="">
                        <img class="w-[20px] h-[20px] sm:w-[30px] sm:h-[30px] lg:w-[40px] lg:h-[40px] justify-self-center xl:justify-self-auto"
                            src="images/social/messenger.png" alt="">
                        <img class="w-[20px] h-[20px] sm:w-[30px] sm:h-[30px] lg:w-[40px] lg:h-[40px] justify-self-center xl:justify-self-auto"
                            src="images/social/instagram 1.png" alt="">
                        <img class="w-[20px] h-[20px] sm:w-[30px] sm:h-[30px] lg:w-[40px] lg:h-[40px] justify-self-center xl:justify-self-auto"
                            src="images/social/linkedin 2.png" alt="">
                        <img class="w-[20px] h-[20px] sm:w-[30px] sm:h-[30px] lg:w-[40px] lg:h-[40px] justify-self-center xl:justify-self-auto"
                            src="images/social/youtube.png" alt="">
                        <img class="w-[20px] h-[20px] sm:w-[30px] sm:h-[30px] lg:w-[40px] lg:h-[40px] justify-self-center xl:justify-self-auto"
                            src="images/social/logo 1.png" alt="">
                        <img class="w-[20px] h-[20px] sm:w-[30px] sm:h-[30px] lg:w-[40px] lg:h-[40px] justify-self-center xl:justify-self-auto"
                            src="images/social/tiktok 1.png" alt="">
                        <img class="w-[20px] h-[20px] sm:w-[30px] sm:h-[30px] lg:w-[40px] lg:h-[40px] justify-self-center xl:justify-self-auto"
                            src="images/social/google 1.png" alt="">
                        <img class="w-[20px] h-[20px] sm:w-[30px] sm:h-[30px] lg:w-[40px] lg:h-[40px] justify-self-center xl:justify-self-auto"
                            src="images/social/google.png" alt="">
                        <img class="w-[20px] h-[20px] sm:w-[30px] sm:h-[30px] lg:w-[40px] lg:h-[40px] justify-self-center xl:justify-self-auto"
                            src="images/social/aws.png" alt="">
                    </div>
                </div>
                <div class="h-fit flex flex-wrap md:justify-between gap-y-4 gap-x-1 sm:gap-y-0 sm:gap-x-4">
                    <div
                        class="w-[68vw] sm:w-[30vw] md:w-[19vw] 2xl:w-[18vw] bg-[#EFF4FF] px-[1rem] lg:px-[2rem] pt-[2rem] pb-[2.5rem] rounded-[14px]">
                        <div class="flex flex-wrap grid grid-cols-1 justify-center">
                            <div class="flex flex-wrap justify-center mb-[1rem] sm:mb-[2rem]">
                                <h1 class="text-[13px] sm:text-[1rem] lg:text-[20px] text-[#012F6A] font-bold text-center"
                                    style="font-family: Red Hat Display">Advertising Tools</h1>
                            </div>
                            <div class="flex justify-center">
                                <img class="mr-6 w-[20px] h-[20px] sm:w-[30px] sm:h-[30px] lg:w-[40px] lg:h-[40px] self-end"
                                    src="images/social/ads.png" alt="">
                                <img class="w-[20px] h-[20px] sm:w-[30px] sm:h-[30px] lg:w-[40px] lg:h-[40px]"
                                    src="images/social/ads.png" alt="">
                            </div>
                        </div>
                    </div>
                    <div
                        class="w-[68vw] sm:w-[32vw] md:w-[20vw] 2xl:w-[20.5vw] bg-[#EFF4FF] px-[1rem] lg:px-[3rem] pt-[2rem] pb-[2.5rem] rounded-[14px] justify-self-start">
                        <div class="flex flex-wrap justify-center grid grid-cols-1">
                            <div class="flex flex-wrap justify-center mb-[1rem] sm:mb-[2rem]">
                                <h1 class="text-[13px] sm:text-[1rem] lg:text-[20px] text-[#012F6A] font-bold text-center"
                                    style="font-family: Red Hat Display">Analytic Tools</h1>
                            </div>
                            <div class="flex justify-center">
                                <img class="w-[85px] h-[20px] sm:w-[108px] sm:h-[30px] lg:w-[118px] lg:h-[40px]"
                                    src="images/social/Group 2 (4).png" alt="">
                            </div>
                        </div>
                    </div>
                    <div
                        class="w-[68vw] sm:w-[fill] md:w-[21.5vw] sm:mt-4 md:mt-0 2xl:w-[24.3vw] bg-[#EFF4FF] px-[1rem] lg:px-[3rem] pt-[2rem] pb-[2.5rem] rounded-[14px] justify-self-start">
                        <div class="flex flex-wrap justify-center grid grid-cols-1">
                            <div class="flex flex-wrap justify-center mb-[1rem] sm:mb-[2rem]">
                                <h1 class="text-[13px] sm:text-[1rem] lg:text-[20px] text-[#012F6A] font-bold text-center"
                                    style="font-family: Red Hat Display">CRM Tools</h1>
                            </div>
                            <div class="flex justify-center">
                                <img class="mr-6 md:mr-4 w-[100px] h-[20px] sm:w-[90px] sm:h-[20px] md:w-[80px] md:h-[20px] xl:w-[110px] xl:h-[30px] self-end"
                                    src="images/social/Pipedrive.png" alt="">
                                <img class="w-[100px] h-[20px] sm:w-[90px] sm:h-[20px] md:w-[80px] md:h-[20px] xl:w-[110px] xl:h-[30px]"
                                    src="images/social/HubSpot.png" alt="">
                            </div>
                        </div>
                    </div>
                </div>
                <div class="h-fit flex flex-wrap md:justify-between gap-y-4 gap-x-1 sm:gap-y-0 sm:gap-x-4">
                    <div
                        class="w-[68vw] sm:w-[33vw] md:w-[22.5vw] 2xl:w-[25.3vw] bg-[#EFF4FF] px-[1rem] lg:px-[3rem] pt-[2rem] pb-[2.5rem] rounded-[14px] justify-self-start">
                        <div class="flex flex-wrap justify-center grid grid-cols-1">
                            <div class="flex flex-wrap justify-center mb-[1rem] sm:mb-[2rem]">
                                <h1 class="text-[13px] sm:text-[1rem] lg:text-[20px] text-[#012F6A] font-bold text-center"
                                    style="font-family: Red Hat Display">Design Tools</h1>
                            </div>
                            <div class="flex justify-center">
                                <img class="mr-6 md:mr-4 w-[20px] h-[20px] sm:w-[30px] sm:h-[30px] xl:w-[40px] xl:h-[40px] self-end"
                                    src="images/social/photo-shop.png" alt="">
                                <img class="mr-6 md:mr-4 w-[20px] h-[20px] sm:w-[30px] sm:h-[30px] xl:w-[40px] xl:h-[40px]"
                                    src="images/social/ads-5.png" alt="">
                                <img class="mr-6 md:mr-4 w-[20px] h-[20px] sm:w-[30px] sm:h-[30px] xl:w-[40px] xl:h-[40px] self-end"
                                    src="images/social/ads-4.png" alt="">
                                <img class="w-[20px] h-[20px] sm:w-[30px] sm:h-[30px] xl:w-[40px] xl:h-[40px]"
                                    src="images/social/ads-3.png" alt="">
                            </div>
                        </div>
                    </div>
                    <div
                        class="w-[68vw] sm:w-[29.2vw] md:w-[22vw] 2xl:w-[22.5vw] bg-[#EFF4FF] px-[1rem] lg:px-[3rem] pt-[2rem] pb-[2.5rem] rounded-[14px] justify-self-start">
                        <div class="flex flex-wrap justify-center grid grid-cols-1">
                            <div class="flex flex-wrap justify-center mb-[1rem] sm:mb-[2rem]">
                                <h1 class="text-[13px] sm:text-[1rem] lg:text-[20px] text-[#012F6A] font-bold text-center"
                                    style="font-family: Red Hat Display">Web Development</h1>
                            </div>
                            <div class="flex justify-center">
                                <img class="mr-6 md:mr-4 w-[20px] h-[20px] sm:w-[30px] sm:h-[30px] xl:w-[40px] xl:h-[40px] self-end"
                                    src="images/social/ads-2.png" alt="">
                                <img class="mr-6 md:mr-4 w-[20px] h-[20px] sm:w-[30px] sm:h-[30px] xl:w-[40px] xl:h-[40px]"
                                    src="images/social/word-press.png" alt="">
                                <img class="w-[20px] h-[20px] sm:w-[30px] sm:h-[30px] xl:w-[40px] xl:h-[40px] self-end"
                                    src="images/social/ads.png" alt="">
                            </div>
                        </div>
                    </div>
                    <div
                        class="w-[68vw] sm:w-[fill] md:w-[16vw] sm:mt-4 md:mt-0 2xl:w-[15vw] bg-[#EFF4FF] px-[1rem] lg:px-[2rem] pt-[2rem] pb-[2.5rem] rounded-[14px]">
                        <div class="flex flex-wrap grid grid-cols-1 justify-center">
                            <div class="flex flex-wrap justify-center mb-[1rem] sm:mb-[2rem]">
                                <h1 class="text-[13px] sm:text-[1rem] lg:text-[20px] text-[#012F6A] font-bold text-center"
                                    style="font-family: Red Hat Display">AI Tool</h1>
                            </div>
                            <div class="flex justify-center">
                                <img class="mr-6 w-[20px] h-[20px] sm:w-[30px] sm:h-[30px] lg:w-[40px] lg:h-[40px] self-end"
                                    src="images/social/chat-gpt.png" alt="">
                                <img class="w-[20px] h-[20px] sm:w-[30px] sm:h-[30px] lg:w-[40px] lg:h-[40px]"
                                    src="images/social/ads.png" alt="">
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </section>
    <section class="flex flex-wrap justify-center pb-[3rem]">
        <div class="w-full flex flex-wrap justify-center grid grid-cols-1 items-center max-w-[100vw] p-6 bg-[length:100%_100%] h-[195px] sm:h-[390px] pb-[7rem]"
            style="background: url('{{ asset('images/full-bg.png') }}'); background-repeat: no-repeat; backgroind-size: cover; background-position: center">
            <h2 class="text-white text-[13px] sm:text-[28px] mb-[1rem] md:text-[36px] font-bold text-center self-end">
                It’s a Team Game</h2>
            <div
                class="flex flex-wrap h-fit w-[18rem] sm:w-[37rem] 2xl:w-[37rem] justify-self-center justify-center self-start">
                <h2
                    class="text-white text-[13px] sm:text-[18px] mb-[1rem] md:text-[18px] font-medium mb-6 text-center self-start">
                    How our team helped 100+ businesses to reach their Sales targets with Digital Marketing activities.
                </h2>
            </div>
        </div>
        <div
            class="h-[auto] justify-between grid grid-cols-1 md:grid md:grid-cols-3 max-w-[65vw] min-w-[65vw] sm:w-[70vw] mt-[-3rem] sm:mt-[-7rem]">
            <div
                class="flex justify-center grid grid-cols-1 bg-white mt-[1rem] md:mt-[0rem] w-full md:w-[19vw] px-[1.5rem] max-h-[20px] min-h-fit rounded-[12px] shadow-md">
                <div class="flex grid grid-cols-1 relative top-[-2rem] md:top-[-2.5rem]">
                    <div class="bg-[#2E79F2] max-h-fit max-w-fit justify-self-center flex rounded-full p-[1rem]">
                        <img class="w-[30px] h-[30px] md:w-[40px] md:h-[40px] lg:w-[50px] lg:h-[50px]"
                            src="images/social/icon.png" alt="">
                    </div>
                    <div class="flex justify-center mt-[2.5rem]">
                        <h1 class="text-center text-[14px] md:text-[15px] lg:text-[20px] text-[#012F6A] font-bold"
                            style="font-family: Red Hat Display">Brainstorming</h1>
                    </div>
                    <div class="flex justify-center mt-[1.5rem]">
                        <h2 class="text-center font-medium text-[#012F6A] text-[13px] md:text-[14px] lg:text-[16px]"
                            style="font-family: Red Hat Display">We sit together and storm ideas to develop a robust
                            strategy.</h2>
                    </div>
                </div>
            </div>
            <div
                class="flex justify-center bg-white w-full mt-[3rem] md:mt-[0rem] md:w-[19vw] justify-self-center px-[1.5rem] rounded-[12px] shadow-md">
                <div class="flex grid grid-cols-1 relative top-[-2rem] md:top-[-2.5rem] min-h-fit max-h-fit">
                    <div class="bg-[#2E79F2] max-h-fit max-w-fit justify-self-center flex rounded-full p-[1rem]">
                        <img class="w-[30px] h-[30px] md:w-[40px] md:h-[40px] lg:w-[50px] lg:h-[50px]"
                            src="images/social/team.png" alt="">
                    </div>
                    <div class="flex justify-center mt-[2.5rem] min-h-fit max-h-fit">
                        <h1 class="text-center text-[13px] md:text-[15px] lg:text-[20px] text-[#012F6A] font-bold"
                            style="font-family: Red Hat Display">Data Driven Decision
                            Making</h1>
                    </div>
                    <div class="flex justify-center mt-[1.5rem] min-h-fit max-h-fit">
                        <h2 class="text-center font-medium text-[#012F6A] text-[13px] md:text-[14px] lg:text-[16px]"
                            style="font-family: Red Hat Display">We sit together and storm ideas
                            to develop a robust strategy.</h2>
                    </div>
                </div>
            </div>
            <div
                class="flex justify-center grid grid-cols-1 bg-white w-full mt-[3rem] md:mt-[0rem] md:w-[19vw] px-[1.5rem] justify-self-end max-h-[20px] min-h-fit rounded-[12px] shadow-md">
                <div class="flex grid grid-cols-1 relative top-[-2rem] md:top-[-2.5rem]">
                    <div class="bg-[#2E79F2] max-h-fit max-w-fit justify-self-center flex rounded-full p-[1rem]">
                        <img class="w-[30px] h-[30px] md:w-[40px] md:h-[40px] lg:w-[50px] lg:h-[50px]"
                            src="images/social/announce.png" alt="">
                    </div>
                    <div class="flex justify-center mt-[2.5rem]">
                        <h1 class="text-center text-[13px] md:text-[15px] lg:text-[20px] text-[#012F6A] font-bold"
                            style="font-family: Red Hat Display">Campaign Activations</h1>
                    </div>
                    <div class="flex justify-center mt-[1.5rem]">
                        <h2 class="text-center font-medium text-[#012F6A] text-[13px] md:text-[14px] lg:text-[16px]"
                            style="font-family: Red Hat Display">We deliver our expertise with
                            the latest tools for campaigns.</h2>
                    </div>
                </div>
            </div>
        </div>
    </section>
</section>
